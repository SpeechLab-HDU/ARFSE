...
\begin{abstract}
Traditional generative speech enhancement (SE) models rely on explicit time step embeddings. However, SE is a strongly correlated task where clean and noisy speech share significant structural similarity. We propose the Autonomous Rectified Flow framework to challenge the necessity of temporal conditioning. By using a linear interpolation path, we mathematically reveal that the target vector field is fundamentally time-invariant. We introduce a time-unconditional network that eliminates explicit time step information, inferring the denoising direction solely from the spatial relationship between the current state and the noisy observation. Predicting this target vector field is equivalent to representing the noise distribution. This autonomous design prevents overfitting to temporal paths, significantly improving generation quality, robustness, and inference efficiency. Code is available at \url{https://github.com/zhangwen0821/ARFSE.git}
\end{abstract}

\section{Introduction}
%介绍相关背景生成式语音增强-扩散模型-流模型-指出时间t依赖-提出我们的模型-我们模型的核心创新点-3点
%1.我们提出了自洽流匹配框架
%2.时间解耦的目标向量场等价噪声分布
%3.多步的高性能和单步的高效率以及泛化性 不拉踩 我们的泛化性也很好即可
Speech enhancement (SE) is a fundamental task aimed to recover clean speech from degraded acoustic environments~\cite{loizou2007speech,hendriks2013dft,jiang2023, jiang_unse_taslp, jiang2026selfse}. Recently, generative paradigms, particularly diffusion probabilistic models and score-based generative models, have set new benchmarks in SE by effectively modeling the complex distribution of speech signals~\cite{welker22_interspeech,10149431,ho2020denoising,Lemercier_2023}. Despite their expressive power, two critical bottlenecks hinder their practical deployment: high computational cost during iterative sampling and the prior mismatch problem. The latter arises because standard diffusion processes typically transform clean speech into an uninformative Gaussian prior, whereas in SE, the noisy observation $y$ provides a more structured and task-relevant prior.

To mitigate this prior mismatch, recent research has shifted toward boundary-anchored stochastic processes, such as the Brownian Bridge with Exponential Diffusion (BBED)~\cite{lay23_interspeech}, which constrains the generative trajectory strictly between clean speech and the noisy mixture. Similarly, Flow Matching (FM)~\cite{lipman2023flow,liu2023flow} and FlowSE~\cite{10888274,zhang2026hyflowse} introduce deterministic ordinary differential equation (ODE)~\cite{chen2018neural} paths based on optimal transport. By employing straight-line trajectories, these frameworks successfully reduce the Number of Function Evaluations (NFEs) required for reverse-time integration while maintaining high fidelity. However, despite these advancements, existing models still rely on explicit time step embeddings to modulate the vector field. 

While explicit temporal conditioning is crucial in standard diffusion models to track the vastly changing scale of noise, we argue that such conditioning is not only redundant but potentially detrimental in the specific context of boundary-anchored SE~\cite{pmlr-v267-sun25g}. If the generative path is a straight line~\cite{liu2023flow}, the target velocity vector remains constant throughout the process. When traditional models are explicitly conditioned on $t$, they are forced to memorize specific noise scales along the trajectory, making them vulnerable to trajectory overfitting. If the numerical solver deviates slightly during inference, a time-conditioned model may fail to correct it because the input state no longer matches the expected distribution for that specific $t$. By treating the enhancement process as an autonomous dynamical system, the vector field can be completely decoupled from explicit temporal variables.
...
\begin{figure}[t]
  ...
  \vspace{-12pt}
\end{figure}
In this paper, we propose the Autonomous Rectified Flow (ARF) framework for time-unconditional SE, which bridges the gap between flow matching and direct noise prediction~\cite{10887784}. The contributions of this paper are summarized as:
\begin{itemize}
\item We theoretically reveal the time-invariance of the target vector field for linear-path generative SE, and prove that explicit time step embedding is mathematically redundant for this task.
\item We propose ARF, a novel time-unconditional SE framework built on an autonomous ordinary differential equation system, which fully removes explicit temporal conditioning and learns the static denoising direction equivalent to the underlying noise distribution.
\item ARF enables highly efficient inference; it achieves a competitive PESQ of 3.11 at $NFE=5$, and slashes the RTF to low 0.02 at $NFE=1$, significantly outperforming existing single step generative SE methods.
\end{itemize}
...
To mitigate the prior mismatch inherent in traditional score-based generative models for SE, this methodology adopts a forward process based on a Brownian bridge~\cite{lay23_interspeech}. Unlike previous stochastic differential equations (SDEs) where the mean only converges to the noisy mixture in the limit, a Brownian bridge is characterized by fixed starting and end points. Using a linear interpolation factor $k(t)=t$ for $t\in[0,1)$, the SDE mean is defined as:
\begin{equation}
    \mu(t) = (1 - t)x_0 + ty
\end{equation}
...
\end{equation}
where $\phi_{0}(x_{0}|y)=x_{0}$ represents the initial state sampled from a conditional prior $p(x_0|y)$, and $v_t$ denotes the time-dependent vector field. Inspired by Optimal Transport (OT), the framework defines a Gaussian conditional probability path with a linear mean and standard deviation:
\begin{equation}
...
\begin{table*}[t]
  ...
  \begin{tabular}{l c c c c c c c c c}
    \toprule
    \textbf{Method} & \textbf{NFEs} & \textbf{PESQ} $\uparrow$ & \textbf{ESTOI} $\uparrow$ & \textbf{SISDR} $\uparrow$ & \textbf{WV-MOS} $\uparrow$ & \textbf{DNSMOS} $\uparrow$ & \textbf{SIG} $\uparrow$ & \textbf{BAK} $\uparrow$ & \textbf{OVRL} $\uparrow$ \\
    \midrule
...
\begin{tabular}{l c c c c c c c c c }
\toprule
\multirow{2}{*}{\textbf{Method}} & \multirow{2}{*}{\textbf{NFEs}} & \multicolumn{4}{c}{\textbf{no\_reverb}} & \multicolumn{4}{c}{\textbf{with\_reverb}} \\
\cmidrule(lr){3-6} \cmidrule(lr){7-10}
& \textbf{} & \textbf{PESQ} $\uparrow$ & \textbf {ESTOI} $\uparrow$ & \textbf{SISDR} $\uparrow$ & \textbf{DNSMOS} $\uparrow$ & \textbf{PESQ} $\uparrow$ & \textbf{ESTOI} $\uparrow$ & \textbf{SISDR} $\uparrow$ & \textbf{DNSMOS} $\uparrow$ \\
\midrule
\multirow{3}{*}{FLOWSE}
...
\toprule
\textbf{Method} & \textbf{NFEs} & \textbf{PESQ} $\uparrow$ & \textbf{ESTOI} $\uparrow$ & \textbf{SISDR} $\uparrow$ & \textbf{RTF} $\downarrow$ \\
\midrule
...
The ARF framework models the generative process by leveraging the time-invariance of a denoising target vector field. For a clean speech $x_0$ and a noisy observation $y$, we define a straight path connecting $x_1$ and $y$:
%关于这里的符号问题 流模型基于扩散的不一样
\begin{equation}
    x_t = (1 - t)x_0 + t(y + \sigma z), \quad z \sim \mathcal{N}(0, I)
\end{equation}
where $z$ is a Gaussian noise used for stochastic regularization. The target update at each intermediate state is then:
\begin{equation}
    u = \frac{d}{dt}x_t = (y + \sigma z) - x_1
\end{equation}
...
\end{equation}
It is worth noting that while $v_\theta$ does not take $t$ as an explicit input, the predicted velocity dynamically evolves along the trajectory because the spatial state $\phi_t$ changes at each step. To solve this autonomous ODE numerically, we adopt a multi-step Euler solver with a uniform sampling schedule. The discrete integration steps $1 = t_0 > t_1 > \dots > t_N = 0$ are distributed as:
\begin{equation}
...
\section{Experiments}%继续完善即可
\subsection{Datasets}
Our experiments are conducted on the VoiceBank+DEMAND~\cite{botinhao2016investigating} dataset, which consists of clean speech from VCTK~\cite{veaux2013voice} mixed with noise from DEMAND~\cite{thiemann2013diverse}. Additionally, to evaluate the generalization capability of our proposed model, we employ the publicly synthetic test set from the DNS Challenge-Interspeech 2020~\cite{reddy20_interspeech} for comprehensive comparison.

% We conduct experiment using the VCTK + DEMAND dataset \cite{Valentini2016}. The dataset consists of two training subsets, and we utilize the one with 28 speakers, containing 11,572 utterances. We randomly select 1,000 utterances for validation and use the remaining 10,572 for training. The test set comprises 824 utterances from two speakers not included in the training data. In the training subset, noisy utterances are mixed at signal-to-noise ratios (SNRs) ranging from 0 dB to 15 dB in 5 dB increments, whilst the test subset contains utterances with SNRs of {2.5, 7.5, 12.5, 17.5} dB. 
...
All models are trained with the Adam optimizer with a learning rate of $1 \times 10^{-4}$ and a batch size of 4. The path scaling parameter $\sigma$ is set to 0.5, and Exponential Moving Average (EMA) is applied to the parameters with a decay factor of 0.999 to stabilize training. For short-time Fourier transform (STFT) features, the FFT size and hop length are set to 510 and 128, respectively. Each model is trained for 100 epochs on a single NVIDIA RTX 3090 GPU.
...
Performance is evaluated using standard metrics including PESQ~\cite{recommendation2001perceptual} scores,  wav2vec MOS (WVMOS)~\cite{andreev2023hifi++}, eSTOI~\cite{jensen2016algorithm}, DNSMOS P.835~\cite{reddy2022dnsmos} including SIG, BAK, OVRL, Scale-Invariant Signal-to-Distortion Ratio (SI-SDR)~\cite{le2019sdr} and Real-Time Factor (RTF). The RTF is mathematically defined as:
\begin{equation}
...
At $NFE=1$, ARFSE (w/o $t$) achieves a PESQ of 2.97 and an ESTOI of 0.88, whereas FlowSE (w/ $t$) and the MeanFlowSE baseline yield PESQ scores of 2.87 and 2.94, respectively. Concurrently, by removing the architectural modules associated with temporal conditioning and time-step modulation layers, ARFSE reduces the Real-Time Factor (RTF) to 0.02 at $NFE=1$. This corresponds to a lower RTF compared to FlowSE (0.05) and MeanFlowSE (0.11), providing empirical support for the hypothesis that explicit temporal conditioning is mathematically redundant for linear-path speech enhancement.

\section{Conclusion}
In this paper, we proposed Autonomous Rectified Flow, a time-unconditional generative speech enhancement framework. By formulating an autonomous ODE system, ARF removes the need for traditional timestep embeddings. We theoretically proved that under a linear interpolation path, the target vector field is fundamentally time-invariant and directly equivalent to the underlying noise distribution. Experimental evaluations show that ARFSE achieves competitive speech quality at $NFE=5$ (PESQ 3.11) and delivers top-tier performance for single-step generation ($NFE=1$) with an remarkably low Real-Time Factor of 0.02. In the future, we will further evaluate the performance of our framework in universal speech enhancement.
