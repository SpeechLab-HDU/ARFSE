...
% We illustrate the conceptual difference between our method and prior arts in Figure~\ref{fig:model}. As shown in Figure~\ref{fig:model}(a)-(b), traditional Rectified Flow and Conditional Flow Matching methods rely on an explicit time step or noise schedule to guide the network in predicting the denoising direction. In contrast, in high-dimensional speech signals, each noisy observation inherently contains enough information to infer its noise level~\cite{pmlr-v267-sun25g}. Leveraging this property, we propose the Autonomous Rectified Flow (ARF) framework (Figure~\ref{fig:model}(c)), which removes the explicit time embedding and predicts the denoising direction solely from the current noisy state and the mixture signal.

We illustrate the conceptual differences between our method and prior approaches in Figure~\ref{fig:model}. As shown in Figure~\ref{fig:model}(a)–(b), traditional Rectified Flow and Conditional Flow Matching methods rely on explicit time-step embeddings or noise schedules to guide the network in predicting the denoising direction. In contrast, high-dimensional speech signals inherently contain sufficient information for inferring the noise level from a noisy observation~\cite{pmlr-v267-sun25g}. Leveraging this property, we propose the Autonomous Rectified Flow framework (Figure~\ref{fig:model}(c)), which removes explicit temporal conditioning and predicts the denoising direction solely from the current state and the noisy mixture.
